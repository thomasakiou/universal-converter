...
const pressureUnits: PressureUnit[] = [
    { code: 'Pa', name: 'Pascals', toPascals: 1 },
    { code: 'kPa', name: 'Kilopascals', toPascals: 1000 },
    { code: 'bar', name: 'Bars', toPascals: 100000 },
    { code: 'psi', name: 'PSI', toPascals: 6894.76 },
    { code: 'atm', name: 'Atmospheres', toPascals: 101325 },
    { code: 'Torr', name: 'Torr', toPascals: 133.322 },
];
...
const PressureConverter: React.FC = () => {
    ...
    useEffect(() => {
        const fromUnitData = pressureUnits.find(u => u.code === fromUnit);
        const toUnitData = pressureUnits.find(u => u.code === toUnit);

        if (fromUnitData && toUnitData) {
            const pascals = amount * fromUnitData.toPascals;
            const converted = pascals / toUnitData.toPascals;
            setResult(converted);
        }
        ...
        <div className="flex flex-col min-h-screen bg-conv-bg font-display text-text-primary transition-colors duration-300">
            ...
            <main className="w-full flex-grow flex flex-col items-center justify-center p-4">
                <div className="flex w-full max-w-2xl flex-col gap-2 p-4 text-center mb-8">
                    <h1 className="text-text-primary text-4xl sm:text-5xl font-black leading-tight tracking-[-0.033em]">Pressure Converter</h1>
                    <p className="text-text-secondary text-base font-normal leading-normal">Pascal, bar, psi, atm</p>
                </div>
                <div className="w-full max-w-2xl rounded-xl bg-conv-card shadow-2xl overflow-hidden transition-colors duration-300">
                            ...
                            <label className="flex flex-col flex-1">
                                ...
                                <div className="relative w-full">
                                    <select value={fromUnit} onChange={(e) => setFromUnit(e.target.value)}
                                        ...
                                        {pressureUnits.map(u => <option key={u.code} value={u.code} className="bg-conv-card text-text-primary">{u.code} - {u.name}</option>)}
                                    </select>
                    ...
                    <div className="bg-input-bg px-6 py-3 flex items-center justify-between text-xs text-text-muted border-t border-conv-border transition-colors">
                        <div className="flex items-center gap-2">
                            <span className="material-symbols-outlined text-sm text-green-400">compress</span>
                    ...
                    <div className="p-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                            <div className="bg-input-bg rounded-lg p-3 border border-conv-border transition-colors">
                                <p className="text-text-secondary mb-1">Atmospheric Pressure</p>
                                <p className="text-text-primary">1 atm = 101,325 Pa</p>
                                <p className="text-text-primary">1 atm ≈ 14.7 psi</p>
                            </div>
                            <div className="bg-input-bg rounded-lg p-3 border border-conv-border transition-colors">
                                <p className="text-text-secondary mb-1">Metric Units</p>
                                <p className="text-text-primary">1 bar = 100,000 Pa</p>
                                <p className="text-text-primary">1 kPa = 1,000 Pa</p>
                            </div>
                            <div className="bg-input-bg rounded-lg p-3 border border-conv-border transition-colors">
                                <p className="text-text-secondary mb-1">Conversions</p>
                                <p className="text-text-primary">1 bar ≈ 14.5 psi</p>
                                <p className="text-text-primary">1 Torr ≈ 133.3 Pa</p>
                            </div>
                            <div className="bg-input-bg rounded-lg p-3 border border-conv-border transition-colors">
                                <p className="text-text-secondary mb-1">Fun Fact</p>
                                <p className="text-text-primary">Car tires are usually around 30-35 psi</p>
                            </div>
                        </div>
                    </div>
                </div>
