...
const powerUnits: PowerUnit[] = [
    { code: 'W', name: 'Watts', toWatts: 1 },
    { code: 'kW', name: 'Kilowatts', toWatts: 1000 },
    { code: 'MW', name: 'Megawatts', toWatts: 1000000 },
    { code: 'hp', name: 'Horsepower (Mechanical)', toWatts: 745.7 },
    { code: 'hp-m', name: 'Horsepower (Metric)', toWatts: 735.5 },
    { code: 'ft-lb/s', name: 'Foot-pounds per Second', toWatts: 1.35582 },
    { code: 'BTU/h', name: 'BTU per Hour', toWatts: 0.293071 },
];
...
const PowerConverter: React.FC = () => {
    ...
    useEffect(() => {
        const fromUnitData = powerUnits.find(u => u.code === fromUnit);
        const toUnitData = powerUnits.find(u => u.code === toUnit);

        if (fromUnitData && toUnitData) {
            const watts = amount * fromUnitData.toWatts;
            const converted = watts / toUnitData.toWatts;
            setResult(converted);
        }
        ...
        <div className="flex flex-col min-h-screen bg-conv-bg font-display text-text-primary transition-colors duration-300">
            ...
            <main className="w-full flex-grow flex flex-col items-center justify-center p-4">
                <div className="flex w-full max-w-2xl flex-col gap-2 p-4 text-center mb-8">
                    <h1 className="text-text-primary text-4xl sm:text-5xl font-black leading-tight tracking-[-0.033em]">Power Converter</h1>
                    <p className="text-text-secondary text-base font-normal leading-normal">Watts, horsepower, BTU/h</p>
                </div>
                <div className="w-full max-w-2xl rounded-xl bg-conv-card shadow-2xl overflow-hidden transition-colors duration-300">
                            ...
                            <label className="flex flex-col flex-1">
                                ...
                                <div className="relative w-full">
                                    <select value={fromUnit} onChange={(e) => setFromUnit(e.target.value)}
                                        ...
                                        {powerUnits.map(u => <option key={u.code} value={u.code} className="bg-conv-card text-text-primary">{u.code} - {u.name}</option>)}
                                    </select>
                    ...
                    <div className="bg-input-bg px-6 py-3 flex items-center justify-between text-xs text-text-muted border-t border-conv-border transition-colors">
                        <div className="flex items-center gap-2">
                            <span className="material-symbols-outlined text-sm text-green-400">electric_bolt</span>
                    ...
                    <div className="p-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                            <div className="bg-input-bg rounded-lg p-3 border border-conv-border transition-colors">
                                ...
                                <p className="text-text-primary">1 kW = 1,000 W</p>
                                <p className="text-text-primary">1 MW = 1,000 kW</p>
                            </div>
                            <div className="bg-input-bg rounded-lg p-3 border border-conv-border transition-colors">
                                <p className="text-text-secondary mb-1">Horsepower</p>
                                <p className="text-text-primary">1 hp ≈ 746 W</p>
                                <p className="text-text-primary">1 hp ≈ 0.746 kW</p>
                            </div>
                            <div className="bg-input-bg rounded-lg p-3 border border-conv-border transition-colors">
                                <p className="text-text-secondary mb-1">Conversions</p>
                                <p className="text-text-primary">1 kW ≈ 1.341 hp</p>
                                <p className="text-text-primary">1 BTU/h ≈ 0.293 W</p>
                            </div>
                            <div className="bg-input-bg rounded-lg p-3 border border-conv-border transition-colors">
                                <p className="text-text-secondary mb-1">Fun Fact</p>
                                <p className="text-text-primary">1 hp was originally defined by lifting coal</p>
                            </div>
                        </div>
                    </div>
                </div>
