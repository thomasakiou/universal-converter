...
const weightUnits: WeightUnit[] = [
    { code: 'mg', name: 'Milligrams', toGrams: 0.001 },
    { code: 'g', name: 'Grams', toGrams: 1 },
    { code: 'kg', name: 'Kilograms', toGrams: 1000 },
    { code: 't', name: 'Metric Tons', toGrams: 1000000 },
    { code: 'oz', name: 'Ounces', toGrams: 28.3495 },
    { code: 'lb', name: 'Pounds', toGrams: 453.592 },
    { code: 'st', name: 'Stones', toGrams: 6350.29 },
    { code: 'ton', name: 'US Tons', toGrams: 907185 },
];
...
const WeightConverter: React.FC = () => {
    ...
    useEffect(() => {
        const fromUnitData = weightUnits.find(u => u.code === fromUnit);
        const toUnitData = weightUnits.find(u => u.code === toUnit);

        if (fromUnitData && toUnitData) {
            const grams = amount * fromUnitData.toGrams;
        ...
        <div className="flex flex-col min-h-screen bg-conv-bg font-display text-text-primary transition-colors duration-300">
            ...
            <main className="w-full flex-grow flex flex-col items-center justify-center p-4">
                <div className="flex w-full max-w-2xl flex-col gap-2 p-4 text-center mb-8">
                    <h1 className="text-text-primary text-4xl sm:text-5xl font-black leading-tight tracking-[-0.033em]">Weight Converter</h1>
                    <p className="text-text-secondary text-base font-normal leading-normal">Grams, pounds, ounces, and more</p>
                </div>
                <div className="w-full max-w-2xl rounded-xl bg-conv-card shadow-2xl overflow-hidden transition-colors duration-300">
                            ...
                            <label className="flex flex-col flex-1">
                                ...
                                <div className="relative w-full">
                                    <select value={fromUnit} onChange={(e) => setFromUnit(e.target.value)}
                                        ...
                                        {weightUnits.map(u => <option key={u.code} value={u.code} className="bg-conv-card text-text-primary">{u.code} - {u.name}</option>)}
                                    </select>
                    ...
                    <div className="bg-input-bg px-6 py-3 flex items-center justify-between text-xs text-text-muted border-t border-conv-border transition-colors">
                        <div className="flex items-center gap-2">
                            <span className="material-symbols-outlined text-sm text-green-400">scale</span>
                    ...
                    <div className="p-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                            <div className="bg-input-bg rounded-lg p-3 border border-conv-border transition-colors">
                                ...
                                <p className="text-text-primary">1 kg = 1,000 g</p>
                                <p className="text-text-primary">1 t = 1,000 kg</p>
                                <p className="text-text-primary">1 g = 1,000 mg</p>
                            </div>
                            <div className="bg-input-bg rounded-lg p-3 border border-conv-border transition-colors">
                                <p className="text-text-secondary mb-1">Imperial Units</p>
                                <p className="text-text-primary">1 lb = 16 oz</p>
                                <p className="text-text-primary">1 st = 14 lb</p>
                                <p className="text-text-primary">1 ton = 2,000 lb</p>
                            </div>
                            <div className="bg-input-bg rounded-lg p-3 border border-conv-border transition-colors">
                                <p className="text-text-secondary mb-1">Conversions</p>
                                <p className="text-text-primary">1 kg ≈ 2.205 lb</p>
                                <p className="text-text-primary">1 oz ≈ 28.35 g</p>
                            </div>
                            <div className="bg-input-bg rounded-lg p-3 border border-conv-border transition-colors">
                                <p className="text-text-secondary mb-1">Fun Fact</p>
                                <p className="text-text-primary">A liter of water weighs exactly 1 kg</p>
                            </div>
                        </div>
                    </div>
                </div>
