...
const timeUnits: TimeUnit[] = [
    { code: 'ms', name: 'Milliseconds', toSeconds: 0.001 },
    { code: 's', name: 'Seconds', toSeconds: 1 },
    { code: 'min', name: 'Minutes', toSeconds: 60 },
    { code: 'h', name: 'Hours', toSeconds: 3600 },
    { code: 'd', name: 'Days', toSeconds: 86400 },
    { code: 'wk', name: 'Weeks', toSeconds: 604800 },
    { code: 'mo', name: 'Months (Avg)', toSeconds: 2628000 },
    { code: 'yr', name: 'Years (Avg)', toSeconds: 31536000 },
];

const TimeConverter: React.FC = () => {
    const { settings } = useSettings();
    const [amount, setAmount] = useState<number>(1);
    const [fromUnit, setFromUnit] = useState<string>('min');
    const [toUnit, setToUnit] = useState<string>('s');
    const [result, setResult] = useState<number>(0);

    useEffect(() => {
        const fromUnitData = timeUnits.find(u => u.code === fromUnit);
        const toUnitData = timeUnits.find(u => u.code === toUnit);

        if (fromUnitData && toUnitData) {
            const seconds = amount * fromUnitData.toSeconds;
            const converted = seconds / toUnitData.toSeconds;
            setResult(converted);
        }
        ...
        <div className="flex flex-col min-h-screen bg-conv-bg font-display text-text-primary transition-colors duration-300">
            ...
            <main className="w-full flex-grow flex flex-col items-center justify-center p-4">
                <div className="flex w-full max-w-2xl flex-col gap-2 p-4 text-center mb-8">
                    <h1 className="text-text-primary text-4xl sm:text-5xl font-black leading-tight tracking-[-0.033em]">Time Converter</h1>
                    <p className="text-text-secondary text-base font-normal leading-normal">Seconds, minutes, hours, and more</p>
                </div>
                <div className="w-full max-w-2xl rounded-xl bg-conv-card shadow-2xl overflow-hidden transition-colors duration-300">
                            ...
                            <label className="flex flex-col flex-1">
                                ...
                                <div className="relative w-full">
                                    <select value={fromUnit} onChange={(e) => setFromUnit(e.target.value)}
                                        ...
                                        {timeUnits.map(u => <option key={u.code} value={u.code} className="bg-conv-card text-text-primary">{u.code} - {u.name}</option>)}
                                    </select>
                    ...
                    <div className="bg-input-bg px-6 py-3 flex items-center justify-between text-xs text-text-muted border-t border-conv-border transition-colors">
                        <div className="flex items-center gap-2">
                            <span className="material-symbols-outlined text-sm text-green-400">schedule</span>
                    ...
                    <div className="p-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                            <div className="bg-input-bg rounded-lg p-3 border border-conv-border transition-colors">
                                <p className="text-text-secondary mb-1">Common Units</p>
                                <p className="text-text-primary">1 min = 60 s</p>
                                <p className="text-text-primary">1 h = 60 min</p>
                                <p className="text-text-primary">1 d = 24 h</p>
                            </div>
                            <div className="bg-input-bg rounded-lg p-3 border border-conv-border transition-colors">
                                <p className="text-text-secondary mb-1">Calendar</p>
                                <p className="text-text-primary">1 wk = 7 d</p>
                                <p className="text-text-primary">1 yr ≈ 365.25 d</p>
                            </div>
                            <div className="bg-input-bg rounded-lg p-3 border border-conv-border transition-colors">
                                <p className="text-text-secondary mb-1">Conversions</p>
                                <p className="text-text-primary">1 h = 3,600 s</p>
                                <p className="text-text-primary">1 d = 86,400 s</p>
                            </div>
                            <div className="bg-input-bg rounded-lg p-3 border border-conv-border transition-colors">
                                <p className="text-text-secondary mb-1">Fun Fact</p>
                                <p className="text-text-primary">A jiffy is an actual unit of time (1/100th sec)</p>
                            </div>
                        </div>
                    </div>
                </div>
